...
//
//  Diffuse Cauchy priors on parameters 
//  except lambda with informative normal prior
//  and equal values for dT and dI
//
...
parameters {
  ...
  real <lower=0> dT;     // death rate of uninfected cells (and infected cells)
...
model {
  ...
  for(n in 1:N) {
    while( t < time[n]){
      ...
      inext = icurr + beta * tcurr .* vcurr - dT * icurr;  // note dI changed to dT
